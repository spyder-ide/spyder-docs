...
def generate_redirect(
    canonical_path, canonical_dir, redirect_dir="", base_path="", base_url="/"
):
    ...
    redirect_output_path = redirect_dir / canonical_path.relative_to(
        canonical_dir
    )
    redirect_url = Path(
        os.path.relpath(canonical_path, start=redirect_output_path.parent)
    )
    ...
    with open(
        redirect_output_path, mode="w", encoding="utf-8", newline="\n"
    ) as redirect_file:
...
def generate_arg_parser():
    ...
    arg_parser.add_argument(
        "--base-url",
        help=("Full URL for the site, for use with the canonical tag."),
    )
    arg_parser.add_argument(
        "--match-glob",
        help=(
    ...
    arg_parser.add_argument(
        ...
        "--verbose",
        action="store_true",
        help=("If passed, prints each redirect generated."),
    )
