...
.. dropdown:: Transcript
   ...
   .. div:: scroll
      ...
      * The third way is to manually enter the path of a file we'd like to check in the path entry box in the pane's toolbar, and click the Analyze button in the pane.
      ...
      For example, the results tell us that there is a warning on line 20. To go directly to this line in the Editor, just click the message. Here, the code analysis says there is a ``bad-whitespace`` issue. To understand what this means, open the Pylint documentation. On the `Pylint docs page`_ http://pylint.pycqa.org/en/stable/index.html), click on Pylint features and search for the code of the message.
