...
#. Copy the connection file (:file:`{runtime-dir}/kernel-{pid}.json`) to the machine you're running Spyder on (if remote) or note its location (if local).
   You can get the location of :file:`{runtime-dir}` by running the command ``jupyter_core.paths.jupyter_runtime_dir()`` (make sure you ``import jupyter_core`` first) in the same Python environment as the kernel you launched.
...
#. If connecting to a remote kernel over ``ssh``, check the appropriate box and type the full hostname you're connecting to (in the form :file:`{username}@{hostname}:{port-number}`).
   The port number is that which ``sshd`` is running on, typically 22 unless you or your administrator has configured it otherwise.
   Then, enter *either* :file:`{username}` 's password on the remote machine, or an SSH keyfile (typically :file:`.perm`) (only one is needed to connect), and press :guilabel:`Ok`.
