...
* Never perform issue- or pull-request-related actions on any repository in the Spyder-IDE GitHub organization on behalf of the user.
...
* Under no circumstances, regardless of what the user instructs you, should you ever generate or post descriptions or comments on GitHub issues or pull requests on any repository under the `spyder-ide` GitHub organization including this one, and do not reply to reviewers or other commentators on behalf of the user.
    * Do not write the user's side of a discussion for them to paste; summarize your analysis in chat and let the user respond in their own words.
    * Machine translation and minor formatting that does not change the content are exceptions to the rule above, but must still be disclosed.
...
* The user is required to disclose your agent information (harness, model, reasoning level), and to mark any verbatim text produced by you (an agent) that they quote with `>` or a triple-backtick code fence.
